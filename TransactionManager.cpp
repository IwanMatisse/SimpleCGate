...
{	
	...
	struct FutDelOrder* del_msg = (struct FutDelOrder*)msg->data;
	strcpy_s(del_msg->broker_code, broker_code_);	

	msg->user_id = order->user_id();
	del_msg = (struct FutDelOrder*)msg->data;
...
{	
	...
	struct FutMoveOrder* move_msg = (struct FutMoveOrder*)msg->data;
	strcpy_s(move_msg->broker_code, broker_code_);	

	msg->user_id = new_order->user_id();
	move_msg = (struct FutMoveOrder*)msg->data;
	move_msg->regime = 1;
...
struct cg_msg_t * TransactionManager::PrepareFutAddOrder(Order* order)
{
	...
	ord = (struct _FutAddOrder*)msg->data;
