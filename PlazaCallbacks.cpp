...
CG_RESULT PlazaConnector::ListenerPartStream(cg_conn_t* conn, cg_listener_t* listener, struct  cg_msg_t* msg)
{
	...
	case CG_MSG_STREAM_DATA:
	{
		...
		{
			plaza_part* part = (plaza_part*)replmsg->data;
...
CG_RESULT PlazaConnector::ListenerFutInfoStream(cg_conn_t* conn, cg_listener_t* listener, struct  cg_msg_t* msg)
{
	...
	case CG_MSG_STREAM_DATA:
	{
		...
		if (replmsg->msg_index == 0)
		{
			...
			plaza_fut_sess_contents* plaza_data = (plaza_fut_sess_contents*)replmsg->data;
...
CG_RESULT PlazaConnector::ListenerFutCommonStream(cg_conn_t* conn, cg_listener_t* listener, struct  cg_msg_t* msg)
{
	...
	case CG_MSG_STREAM_DATA:
	{
		...
		if (replmsg->msg_index == 0)
		{
			...
			plaza_fut_common* plaza_common = static_cast<plaza_fut_common*>(replmsg->data);
...
CG_RESULT PlazaConnector::ListenerVMStream(cg_conn_t* conn, cg_listener_t* listener, struct  cg_msg_t* msg)
{
	...
	case CG_MSG_STREAM_DATA:
	{
		...
		if (replmsg->msg_index == 0 )
		{
			plaza_fut_vm* plaza_vm = (plaza_fut_vm*)replmsg->data;
...
CG_RESULT PlazaConnector::ListenerPositionStream(cg_conn_t* conn, cg_listener_t* listener, struct  cg_msg_t* msg)
{
	...
	case CG_MSG_STREAM_DATA:
	{
		...
		if (replmsg->msg_index == 0)
		{
			plaza_position* plaza_pos = static_cast<plaza_position*>(replmsg->data);
...
CG_RESULT PlazaConnector::ListenerFutTradeStream(cg_conn_t* conn, cg_listener_t* listener, struct  cg_msg_t* msg)
{
	...
	case CG_MSG_STREAM_DATA:
	{
		...
		case 0://orders
		{
			plaza_my_orders_log *plaza_order = static_cast<plaza_my_orders_log*>(replmsg->data);
		...
		{
			plaza_my_multileg_orders_log *plaza_order = (plaza_my_multileg_orders_log*)replmsg->data;
		...
		{
			plaza_multileg_deal *plaza_trade = (plaza_multileg_deal*)replmsg->data;
		...
		case 3: //deals
		{
			plaza_user_deal *plaza_trade = (plaza_user_deal*)(replmsg->data);
			if (plaza_trade->ext_id_sell == plaza_trade->ext_id_buy) 
				break;
			if (database_.IsTradeExist(plaza_trade->id_deal))
				break;
...
CG_RESULT PlazaConnector::PublisherCallback(cg_conn_t* conn, cg_listener_t* listener, struct cg_msg_t* msg)
{
	...
	case CG_MSG_DATA:
	{
		...
		case 109: //ADD
		{
			code = ((FORTS_MSG101*)((struct cg_msg_data_t*)msg)->data)->code;
			...
			if (database_.IsOrderExist(user_id))
			{
				...
				if (code == 0)
				{					
					order->SetId(((FORTS_MSG101*)((struct cg_msg_data_t*)msg)->data)->order_id);
				}
				else
				{
					order->SetState(kFailed);
					AddOrderEvent(order);
					printf("ORDER FAILED: %s \n", ((FORTS_MSG101*)((struct cg_msg_data_t*)msg)->data)->message);
				}
			}
		...
		case 110:		//DELETE
		{
			code = ((FORTS_MSG102*)((struct cg_msg_data_t*)msg)->data)->code;
			if (database_.IsOrderExist(user_id))
			{
				auto order = database_.GetOrder(user_id);
				if (code == 0)
				...
				{					
					printf("ORDER DELETE FAILED: %s \n", ((FORTS_MSG102*)((struct cg_msg_data_t*)msg)->data)->message);
				}
			}
			break;
		}
		...
		case 113: //MOVE
		{
			code = ((FORTS_MSG105*)((struct cg_msg_data_t*)msg)->data)->code;
			...
			if (database_.IsOrderExist(user_id))
			{
				...
				if (code == 0)
				{					
					order->SetId(((FORTS_MSG105*)((struct cg_msg_data_t*)msg)->data)->order_id1);
					if (order->id() == 0)
					{						
				...
				{					
					...
					if (code == 50)
					{
						...
						((FORTS_MSG105*)((struct cg_msg_data_t*)msg)->data)->code,
							((FORTS_MSG105*)((struct cg_msg_data_t*)msg)->data)->order_id1,
							((FORTS_MSG105*)((struct cg_msg_data_t*)msg)->data)->order_id2);
		...
		case 99://FLOOD
		{			
			FORTS_MSG99* ms = ((FORTS_MSG99*)((struct cg_msg_data_t*)msg)->data);			
			code = 99;
			if (database_.IsOrderExist(user_id))
			{
				auto order = database_.GetOrder(user_id);						
